          ...
          <Menu>
              {menu.map((outerItem, index) => {
                  if (!outerItem.children) {
                    return <SubMenuWithLink {...outerItem} key={index}/>
                  } else {
                      return (<SubMenu>
                          {outerItem.children.map((innerItem, i) => <SubMenuWithLink {...innerItem} key={i}/>)}
                      </SubMenu>)
                  }
              })}
          </Menu>
...
function SubMenuWithLink (name, link, key) {
    return <Menu.Item key={key}><Link to={link}>{name}</Link></Menu.Item>;
}
